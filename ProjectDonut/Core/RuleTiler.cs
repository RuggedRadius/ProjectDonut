...
namespace ProjectDonut.Core
{
    ...
    public class RuleTiler
    {
        ...
        public static Texture2D DetermineBuildingWallCapTexture(int[,] wallMap, int[,] floorMap, Tilemap wallTiles, int x, int y)
        {
            ...
            {
                return SpriteLib.BuildingBlockSprites["wall-cap-sw"];
            }

            if (nb.North && nb.West && !nb.East)
            {
                return SpriteLib.BuildingBlockSprites["wall-cap-se"];
            }

            if (nb.South && nb.East && !nb.West)
            {
                return SpriteLib.BuildingBlockSprites["wall-cap-nw"];
            }

            if (nb.South && nb.West && !nb.East)
            {
                return SpriteLib.BuildingBlockSprites["wall-cap-ne"];
            }
            ...
            if (!nb.North)
            {
                return SpriteLib.BuildingBlockSprites["wall-cap-n"];
            }

            if (!nb.East)
            {
                if (floorMap[x - 1, y] == 1)
                    return SpriteLib.BuildingBlockSprites["wall-cap-e"];

                if (floorMap[x + 1, y] == 1)
                    return SpriteLib.BuildingBlockSprites["wall-cap-w"];
            ...
            if (!nb.South)
            {
                return SpriteLib.BuildingBlockSprites["wall-cap-s"];
            }
        ...
        public static Texture2D DetermineBuildingWallCapTexturePass2(int[,] wallMap, int[,] floorMap, Tilemap wallTiles, int x, int y)
        {
            ...
            if (s == SpriteLib.BuildingBlockSprites["wall-face"])
            {
                if (e == SpriteLib.BuildingBlockSprites["wall-face"])
                    return SpriteLib.BuildingBlockSprites["wall-cap-ne"];

                if (w == SpriteLib.BuildingBlockSprites["wall-face"])
                    return SpriteLib.BuildingBlockSprites["wall-cap-nw"];
        ...
        public static class Town
        {
            public static Texture2D TownBaseTexture(int[,] map, int x, int y)
            {
                ...
                {
                    case 0:
                        return SpriteLib.TownSprites["dirt-c"];

                    case 1:
                        return SpriteLib.TownSprites["dirt-c"];

                    case 2:
                        return SpriteLib.TownSprites["grass-c"];
            ...
            public static Texture2D FenceTexture(int[,] map, int x, int y)
            {
                ...
                if (!n && e && s && !w)
                    return SpriteLib.TownSprites["fence-nw"];
                if (!n && e && !s && w)
                {
                    if (y == 0)
                        return SpriteLib.TownSprites["fence-n"];
                    //else if (map[x, y - 1] == 1)
                ...
                if (!n && !e && s && w)
                    return SpriteLib.TownSprites["fence-ne"];
                if (n && !e && s && !w)
                {
                    if (x == 0)
                        return SpriteLib.TownSprites["fence-w"];
                    //else if (map[x - 1, y] == 1)
                ...
                if (n && e && !s && !w)
                    return SpriteLib.TownSprites["fence-sw"];
                if (n && !e && !s && w)
                    return SpriteLib.TownSprites["fence-se"];
                else
                    return SpriteLib.TownSprites["fence-s"];
            }
