    ...
    public class WaterGenerator
    {
        ...
        private List<(int, int, int)> FindCoastCoords(int[,] heightData)
        {
            ...
            foreach (var waterCoord in waterCoords)
            {
                ...
                var n = heightData[x + 0, y - 1] >= settings.CoastHeightMax;
                var w = heightData[x + 0, y + 1] >= settings.CoastHeightMax;
                var e = heightData[x + 1, y + 0] >= settings.CoastHeightMax;
                var s = heightData[x - 1, y + 0] >= settings.CoastHeightMax;
        ...
        private List<(int, int)> GetAllDeepWaterCoords(int[,] heightData)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                {
                    if (heightData[x, y] <= settings.DeepWaterHeightMax)
                    {
                        coords.Add((x, y));
                    }
                }
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    if (heightData[x, y] <= settings.WaterHeightMax && 
                        heightData[x, y] > settings.DeepWaterHeightMax)
                    {
                        coords.Add((x, y));
                    }
                }
        ...
        private List<(int, int)> GetAllMountainCoords(int[,] heightData)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    if (heightData[x, y] <= settings.MountainHeightMax && 
                        heightData[x, y] > settings.GroundHeightMax)
                    {
                        coords.Add((x, y));
                    }
                }
        ...
        private int IsCoastCoord(int x, int y, int[,] heightData)
        {
            ...
            var isCoastNorth = heightData[x, y - 1] <= settings.CoastHeightMax && heightData[x, y - 1] > settings.WaterHeightMax;
            var isCoastEast = heightData[x + 1, y] <= settings.CoastHeightMax && heightData[x + 1, y ] > settings.WaterHeightMax; 
            var isCoastSouth = heightData[x - 1, y] <= settings.CoastHeightMax && heightData[x - 1, y] > settings.WaterHeightMax;
            var isCoastWest = heightData[x, y + 1] <= settings.CoastHeightMax && heightData[x, y + 1] > settings.WaterHeightMax;
